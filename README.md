...
- `-l`, `--list` – list available information. Valid types: `config`, `editor`, `language`, `model`, `path`, `provider`, `stats`, `style`
  - `--list stats` – show local-only usage analytics (commits/squashes/PRs per provider, tokens, average latency)
  - `--since YYYY-MM-DD` – filter `--list stats` to events on or after this date
  - `--json` – render `--list stats` output as JSON
...
git cai -l stats                 # text summary
git cai -l stats --json          # machine-readable
git cai -l stats --since 2026-01-01  # date filter
git cai -l stats --reset-stats   # wipe all rows
```
