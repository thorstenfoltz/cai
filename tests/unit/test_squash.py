...
def clean_git_state():
    ...
    """
    def _side_effect(cmd, text=True):
...
def test_aborts_if_not_in_git_repo(caplog):
...
def test_aborts_on_unstaged_changes(mock_repo_root):
    with (
...
def test_commits_staged_changes_first(mock_repo_root, mock_generator):
    def git_side_effect(cmd, text=True, **kwargs):
        if cmd[:3] == ["git", "diff", "--cached"]:
            return "file.py"
        if cmd[:2] == ["git", "diff"]:
            return ""
    ...
    with (
        ...
        patch("git_cai_cli.core.squash.commit_with_edit_template", return_value=0) as commit,
        patch("git_cai_cli.core.squash.load_config", return_value={"default": "openai"}),
...
def test_force_push_prompt_and_execution(
    ...
    run_mock.assert_any_call(
        ["git", "push", "--force-with-lease"], check=True
    )
